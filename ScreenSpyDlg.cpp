	...
	: CDialog(CScreenSpyDlg::IDD, pParent)
{
    ...
    m_hCursor=AfxGetApp()->LoadCursor(IDC_CURSOR_CATCH_SCREEN);      
...
void CScreenSpyDlg::saveToFile(CString strSaveFile)
{
	...
	if(strSaveFile=="")
	{
		...
		HBITMAP hBitmap = CopyScreenToBitmap(&m_rcSel[0],TRUE);
		
		if (hBitmap)
		{
			fipWinImage image;
			image.copyFromBitmap(hBitmap);
			image.save(strSaveFile);
		}
	}
...
void CScreenSpyDlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(m_bSelected)
	{
		...
		{//设置为背景
			...
			HBITMAP hBitmap = CopyScreenToBitmap(&m_rcSel[0],TRUE);
			//HBITMAP hBitmap2 = CopyScreenToBitmap(m_RectTracker.m_rect,TRUE);
			if(hBitmap)
			{
				char szCurDir[MAX_PATH]={0};
				GetCurrentDirectory(MAX_PATH,szCurDir);
				...
				/*fipWinImage image2;
				image2.copyFromBitmap(hBitmap2);
				image2.save("c:\\2.bmp");*/
		...
		else if(PtInRect(m_rcAction[1],point))
		{
			saveToFile();
			m_bLeftDown=false;
		}
		else if (PtInRect(m_rcAction[2],point))
		{
